...
## 데이터 파이프라인 구축
agv_dataset = joblib.load('DataFrame/agv_dataframe.pkl') # 데이터 프레임 불러오기
agv_X, agv_y = agv_dataset.drop(columns='state'),agv_dataset['state'] # X,y 분리 (X는 이미지 + 센서 데이터, y는 state(정답값))
_, agv_X_test, _, agv_y_test = train_test_split(agv_X,agv_y,test_size=0.3,shuffle=False) # 모델은 이미 X_train은 학습했으므로 X_test만 필요

oht_dataset = joblib.load('DataFrame/oht_dataframe.pkl') # 데이터 프레임 불러오기
oht_X, oht_y = oht_dataset.drop(columns='state'),oht_dataset['state'] # X,y 분리 (X는 이미지 + 센서 데이터, y는 state(정답값))
